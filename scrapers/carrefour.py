...
def scrape_category_for_store(driver, category_url, store, category_name):
   ...
   def scrape_category_for_store(driver, category_url, store, category_name):
    ...
    cards = soup.find_all("a", class_="product-card-click-wrapper") for card in cards: nom = card.find("h3", class_="product-card-title__text") prix = card.find("span", class_="product-price__content")
